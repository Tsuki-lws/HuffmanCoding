...
void FileIO::compressFile(const string &filename, const string &outputFileName)
{
    ...
    else
    {
        ...
        long long filesize = filehead.originBytes;
        ...
        //     }
        // } else {
        //     while (inputFile.read(inputBuffer.data(), BUFFER_SIZE)) {
        ...
        while(filesize > BUFFER_SIZE){
            inputFile.read(inputBuffer.data(), BUFFER_SIZE);
            filesize -= BUFFER_SIZE;
            for(size_t j = 0; j < 512; j++){
                for (size_t i = 0; i < BUFFER_SIZE/512; i++) {
                    buffer += charCode[inputBuffer[i + j * 512]];
                }
                while (buffer.size() >= 8) {
                    bitset<8> bits(buffer.substr(0, 8));
                    outputBuffer.push_back((char)(bits.to_ulong()));
